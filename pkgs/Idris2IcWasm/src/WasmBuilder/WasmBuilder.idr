...
compileToRefC opts buildDir = do
  ...
  where
    ...
    compileWithIpkg opts' ipkg = do
      ...
      -- Ignoring the CODE is right; discarding the OUTPUT was not. The fork's
      -- `--cg refc` is fail-open (`ignore $ compileExp` in SetOptions.idr /
      -- Package.idr drops the Maybe), so a failed codegen exits 0 with no
      -- artefact — which makes "no artefact" the only signal we get, and the
      -- compiler's own text the only thing that can say why. Measured on carl
      -- 2026-08-07: dfx phase-2 died with a bare "No C file generated by RefC"
      -- and nothing else, on either side of the run, for hours.
    ...
    compileDirectly opts' buildDir' = do
      ...
      _ <- executeCommand clearCmd
      -- Keep the compiler's output for the failure case: see the note on the
      -- other no-C-file branch. The fork exits 0 on a failed refc codegen, so
