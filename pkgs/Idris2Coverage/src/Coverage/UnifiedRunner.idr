...
||| "Coverage.UnifiedRunner" -> "Coverage/UnifiedRunner" (its place under build/ttc/<ver>/).
moduleTtcPath : String -> String
moduleTtcPath m = pack (map (\c => if c == '.' then '/' else c) (unpack m))

||| Remove the TTC/TTM of exactly these modules from the project's build/ttc, so
||| the next build re-elaborates them instead of loading them.
|||
||| Scoped deliberately: the chunk's own modules only. Their transitive closure
||| stays compiled, which is the whole reason the chunk borrows the project's
||| build dir (see the call site). The version directory is a glob because its
||| name is the compiler's TTC version, which this code should not have to know.
|||
||| This mutates shared build state. It is the *cheap* side of a choice — the
||| artefacts deleted here are regenerated by the next build, whereas the
||| alternative (touching the module sources to invalidate them by mtime) would
||| perturb an input that other machinery reads.
evictChunkTtcs : (projectDir : String) -> (modules : List String) -> IO ()
evictChunkTtcs projectDir modules = do
  let ttcRoot = projectDir ++ "/build/ttc"
  let evictOne = \m =>
        let p = moduleTtcPath m in
        "find '" ++ ttcRoot ++ "' \\( -path '*/" ++ p ++ ".ttc' -o -path '*/" ++ p
          ++ ".ttm' \\) -delete 2>/dev/null; "
  _ <- system ("{ " ++ concat (map evictOne modules) ++ ":; } >/dev/null 2>&1")
  dbg <- getEnv "IDRIS2COV_SLICE_DEBUG"
  case dbg of
    Nothing => pure ()
    Just v => if trim v == "" || trim v == "0"
                 then pure ()
                 else ignore (fPutStrLn stderr
                        ("    [chunk] evicted TTC for " ++ show (length modules)
                         ++ " module(s) so --dumppaths-json re-enumerates them"))
...
runStaticDumppathsJsonChunk projectDir sourcedir tempBuildDir projectDepends packTomlContent modules idx = do
  ...
  -- ...but reuse costs the denominator, so evict THIS CHUNK's own TTCs first.
  -- --dumppaths-json enumerates what the running process elaborated. A module
  -- served from a valid TTC is not elaborated, and measurably loses its
  -- USER-DEFINED TOP-LEVEL functions from the dump while its lifted `case block
  -- in ...` children survive — the entry points vanish and only the interiors
  -- remain. Measured 2026-08-07 on a 2-module fixture: cold 7 functions, warm 5,
  -- the missing two being `Helper.classify` and `Helper.twice` (both paths=1
  -- when cold, so this is NOT the known zero-step filter). At package scale the
  -- same effect moved dfx's denominator 3243 -> 2594.
  --
  -- Evicting only the chunk's own modules keeps the transitive closure warm, so
  -- the OOM argument above still holds; and it touches build artefacts rather
  -- than sources. That distinction is the point: `touch`ing the sources would
  -- also force re-elaboration, but the daemon's drift habit fingerprints the
  -- tree with `stat -f '%m %N'`, so measuring coverage would fire a drift pass —
  -- and a --restore pass rewrites tests/specs/docs. Measuring must not mutate
  -- the thing being measured.
  when hasPrebuilt $ evictChunkTtcs projectDir modules
