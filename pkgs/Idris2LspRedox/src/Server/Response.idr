...
||| Sends a success response for a custom method that is not represented in
||| lsp-lib's closed Method type.
export
sendRawResponseMessage : Ref LSPConf LSPConfiguration
                      => JSON
                      -> JSON
                      -> Core ()
sendRawResponseMessage id result = do
  let msg = JObject [("jsonrpc", JString "2.0"), ("id", id), ("result", result)]
  writeResponse msg
  logI Channel "Sent raw custom response message"
  logD Channel "Raw response sent: \{stringify msg}"

||| Sends an error response for a custom method that is not represented in
||| lsp-lib's closed Method type.
export
sendRawResponseError : Ref LSPConf LSPConfiguration
                    => JSON
                    -> ResponseError
                    -> Core ()
sendRawResponseError id error = do
  let msg = JObject [("jsonrpc", JString "2.0"), ("id", id), ("error", toJSON error)]
  writeResponse msg
  logI Channel "Sent raw custom error response message"
  logD Channel "Raw error response sent: \{stringify msg}"
