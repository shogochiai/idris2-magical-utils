...
import Server.Log
import Server.PathSnapshot.Handler
...
handleTypedClientRequest :
    ...
    -> JSON
    -> Core ()
handleTypedClientRequest fields idJSON methodJSON = do
    ...
    Just methodJSON => do -- request or notification
      case lookup "id" fields of
        Just idJSON => do -- request
          case methodJSON of
            JString methodName =>
              if isRedoxPathSnapshotMethod methodName
                 then do
                   logI Channel "Received Redox path snapshot request for method \{methodName}"
                   result <- catch (handlePathSnapshotRequest methodName (lookup "params" fields)) $ \err => do
                     logE Server "Error while handling Redox path snapshot request: \{show err}"
                     resetContext (Virtual Interactive)
                     pure $ Left (MkResponseError (Custom 4) (show err) JNull)
                   case result of
                     Left error => sendRawResponseError idJSON error
                     Right value => sendRawResponseMessage idJSON value
                 else handleTypedClientRequest fields idJSON methodJSON
            _ => handleTypedClientRequest fields idJSON methodJSON
