# idris2-lsp-redox
Language Server for Idris2, forked with Redox path snapshot RPCs.

This fork preserves the standard Idris2 LSP surface and advertises Redox
extensions under the initialize `experimental.redoxPathSnapshots` capability.
The protocol contract is documented in [REDOX.md](REDOX.md); the current schema
is [schema/redox-path-snapshot-1.1.json](schema/redox-path-snapshot-1.1.json).
