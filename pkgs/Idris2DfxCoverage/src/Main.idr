...
runNumeratorSubprocess opts ipkgPath dumppathsContent = do
  ...
  -- BUT the child still needs the fork for path-hits CODEGEN: its
  -- resolvePathcovForkBin reads IDRIS2_PATHCOV_FORK_BIN first, so re-inject the
  -- fork through that seam BEFORE `env -u IDRIS2_BIN` severs the default. On a
  -- host without the /Users/bob legacy literal (alice), dropping IDRIS2_BIN
  -- without this injection kills phase-2 with "No C file generated by RefC".
  mForkEnv <- getEnv "IDRIS2_PATHCOV_FORK_BIN"
  mBinEnv  <- getEnv "IDRIS2_BIN"
  let neTrim : Maybe String -> Maybe String
      neTrim m = case map trim m of
                   Just s => if null s then Nothing else Just s
                   Nothing => Nothing
  let forkInject = case the (Maybe String) (neTrim mForkEnv <|> neTrim mBinEnv) of
                     Just f  => "IDRIS2_PATHCOV_FORK_BIN=\"" ++ f ++ "\" "
                     Nothing => ""
  let cmd = "env -u IDRIS2_BIN " ++ forkInject
